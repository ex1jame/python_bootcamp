...
# ORDER BY Позволяет нам сортировать выводящие данные по убыванию или возрастаю.
# ASC (ПО возрастанию) и DESC  (по убыванию).
# Синтаксис select <row> from <table name> order by  <column name> asc|desc;

# LIMIT Ограничивает количество строк, которое будет возвращаться.
# Синтаксис select <rows> from <table name> limit <number of rows>;

# GROUP BY Группирует результаты по одному из столбцов
...
# SELECT p1.title, p1.price, p1.quantity, p1.price * o1.quantity as total_sum 
# FROM products p1, orders o1 where p1.id = o1.product_id;  -запрос сразу к двум таблицам

# SELECT p1.title, p1.price, p1.quantity, p1.price * o1.quantity as 
